...
result=adapt.integrate(sinx, (0.0,1.0, 2.0, math.sqrt(8*math.pi)),TOL);
diff=abs(PRECISE_RESULT-result)
print("diff={0}".format(diff))
print("Required precision={0}".format(TOL))

n=10
timer=timeit.Timer('integrate(sinx, (0.0,1.0, 2.0, math.sqrt(8*math.pi)),1e-10)', setup="import math; from adapt import integrate, sinx;")
dt=timer.timeit(n)
